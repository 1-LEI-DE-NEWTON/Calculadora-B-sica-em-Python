...
def stringsrapidas():
  ...
  regex_inteiros= '(?<![a-zA-Z:])[-+]?\d*\.?\d+'
  regex_naturais= '^\d+$'
  logd= 'o logaritmo de'
  return digitacao,saindo,resultado, invalido, somente, regex_inteiros, logd, regex_naturais, CE_Fatorial
digitacao, saindo, resultado, invalido, somente, regex_inteiros, logd, regex_naturais, CE_fatorial = stringsrapidas()

class Testes:

  def teste1(numero1): 
    while re.fullmatch(f'{regex_inteiros}', numero1) is None: 
      print(f'{somente}')
      Menu.numeros_basicos1()
    return numero1  
  
  def teste2(numero2): 
    while re.fullmatch(f'{regex_inteiros}', numero2) is None: 
      print(f'{somente}')
      Menu.numeros_basicos2()
    return numero2

  def teste3(numero3): 
    while re.fullmatch(f'{regex_inteiros}', numero3) is None: 
      print(f'{somente}')
      Menu.matematica()
    return numero3   
  
  def teste4(numero4):
    while re.fullmatch(f'{regex_inteiros}', numero4) is None: 
      print(f'{somente}')
      Menu.matematica()
    return numero4

  def teste5(numero5):
    while re.fullmatch(f'{regex_inteiros}', numero5) is None: 
      print(f'{somente}')
      Menu.matematica()
    return numero5
